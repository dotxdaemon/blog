...
assertMatches(css, /background:\s*linear-gradient\(135deg,\s*var\(--paper-soft\)\s*0%,\s*var\(--paper\)\s*100%\)/i, 'Expected gradient background in styles.');
assertMatches(css, /\.module-card[\s\S]*border-radius:\s*var\(--radius-1\)[\s\S]*padding:\s*36px/i, 'Expected glass card style dimensions.');
