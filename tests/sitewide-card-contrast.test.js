...
assertMatches(homepageCss, /\.module-card[\s\S]*border:\s*var\(--border-width\)\s*solid\s*rgba\(255,\s*255,\s*255,\s*0\.15\)/i, 'Expected homepage module cards to use the shared border treatment.');
...
assertMatches(homepageCss, /\.album-item[\s\S]*border:\s*var\(--border-width\)\s*solid\s*var\(--border\)/i, 'Expected homepage album cards to use the shared border treatment.');
