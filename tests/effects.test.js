...
// ABOUTME: Ensures glow, opacity, and transition effects are present and constrained.
...
assertMatches(css, /box-shadow\s*:\s*0\s*8px\s*32px\s*rgba\(0,\s*0,\s*0,\s*0\.4\)/i, 'Expected card drop shadow styling.');
assertMatches(css, /transition\s*:[^;]*cubic-bezier\(0\.4,\s*0,\s*0\.2,\s*1\)/i, 'Expected smooth cubic-bezier transitions.');
assertNotMatches(css, /filter\s*:\s*blur/i, 'Did not expect blur filters in the stylesheet.');
