...
assertMatches(css, /background:\s*linear-gradient\(135deg,\s*var\(--paper-soft\)\s*0%,\s*var\(--paper\)\s*100%\)/i, 'Expected base linear background gradient.');
...
assertMatches(css, /\.module-card[\s\S]*border:\s*var\(--border-width\)\s*solid\s*rgba\(255,\s*255,\s*255,\s*0\.15\)/i, 'Expected micro-borders on cards.');
...
assertMatches(css, /\.section-title[\s\S]*font-family:\s*var\(--font-label\)/i, 'Expected monospace labels for interface metadata.');
assertMatches(css, /\.post-date[\s\S]*color:\s*var\(--muted-strong\)/i, 'Expected deemphasized date styling.');
...
assertMatches(css, /\.post-list::before[\s\S]*var\(--border-width\)\s*solid\s*var\(--line\)/i, 'Expected timeline guide line for recent posts.');
assertMatches(css, /\.post-row:hover\s+\.post-link[\s\S]*color:\s*var\(--accent\)/i, 'Expected hover state to use electric blue accent color.');
