...
assertMatches(css, /\.module-card[\s\S]*background-color:\s*rgba\(0,\s*0,\s*0,\s*0\.85\)[\s\S]*border:\s*1px\s*solid\s*rgba\(255,\s*255,\s*255,\s*0\.12\)[\s\S]*border-radius:\s*16px[\s\S]*padding:\s*32px[\s\S]*backdrop-filter:\s*blur\(12px\)/i, 'Expected glass card baseline styles.');
assertMatches(css, /\.module-card:hover[\s\S]*border-color:\s*rgba\(255,\s*255,\s*255,\s*0\.2\)[\s\S]*box-shadow:\s*0\s*12px\s*48px\s*rgba\(0,\s*0,\s*0,\s*0\.6\)/i, 'Expected hover border/box-shadow lift.');
