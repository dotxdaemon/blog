...
assertMatches(css, /background:\s*linear-gradient\(135deg,\s*var\(--paper-soft\)\s*0%,\s*var\(--paper\)\s*100%\)/i, 'Expected gradient usage in homepage styles.');
assertMatches(css, /\.module-card[\s\S]*background-color:\s*var\(--card-bg\)/i, 'Expected translucent shared card surface.');
