...
assert.ok(css.includes('.panel-row {'), 'Expected panel row styles to be defined.');
assert.ok(
...
assert.ok(
  /\.panel-row[\s\S]*border-bottom:\s*var\(--borderWidth\) solid var\(--border\)/i.test(css),
  'Expected track rows to use the primary divider border color.'
);
assert.ok(
