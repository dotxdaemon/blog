// ABOUTME: Validates homepage tokens and layout maintain the editorial reading direction.
// ABOUTME: Confirms warm palette, serif hierarchy, and balanced two-column desktop composition.
...
assertMatches(css, /\.site-title[\s\S]*letter-spacing:\s*0\.13em/i, 'Expected title letter spacing near +130.');
...
assertMatches(css, /\.listening-to[\s\S]*padding-top:\s*0/i, 'Expected listening module top padding to stay removed so headings align.');
...
assertMatches(css, /\.post-stream\s+\.section-title[\s\S]*margin-bottom:\s*24px/i, 'Expected compact spacing between RECENT POSTS heading and first item.');
assertMatches(css, /\.post-list[\s\S]*gap:\s*32px/i, 'Expected consistent spacing between post items.');
assertMatches(css, /\.post-title[\s\S]*font-size:\s*14pt/i, 'Expected post titles to use refined 14pt type.');
assertMatches(css, /\.post-excerpt[\s\S]*font-size:\s*10pt/i, 'Expected post excerpts to use compact secondary type.');
assertMatches(css, /\.post-date[\s\S]*font-size:\s*8pt/i, 'Expected post dates to use 8pt type.');
assertMatches(css, /\.post-row:hover\s+\.post-title[\s\S]*opacity:\s*0\.82/i, 'Expected post title hover to use a restrained opacity transition.');
